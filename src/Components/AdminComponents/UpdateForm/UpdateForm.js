...
export const getProductos = async () => {
  try {
    const response = await axios.get(`http://ligobas.com:5000/producto`);
    return response;
  } catch (error) {
    console.log(error);
  }
...
export function UpdateForm() {
  ...
  useEffect(() => {
    ...
    async function getPromos() {
      var promos = [];
      let aux = (await axios.get("http://ligobas.com:5000/promotion")).data;
  ...
  const handleChange = (e) => {
   ...
   const editar = async () => {
    ...
    try {
      await axios.put(`http://ligobas.com:5000/producto/${bonsaiSeleccionado._id}`, updatedProducto);
          ...
          <tbody>
            {catalogos.map((elemento) => (
              <tr>
                <td>{elemento._id}</td>
                <td>{elemento.tipo}</td>
                <td><img className="imagenArbol" src={`http://ligobas.com:5000/${elemento.image}`} alt="arbolimagen"></img></td>
