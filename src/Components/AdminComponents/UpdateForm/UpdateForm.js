...
export function UpdateForm() {
  ...
  useEffect(() => {
    ...
    async function getPromos() {
      var promos = [];
      let aux = (await axios.get("http://34.204.6.112:5000/promotion")).data;
  ...
  const handleChange = (e) => {
   ...
   const editar = async () => {
    ...
    try {
      await axios.put(`http://34.204.6.112:5000/producto/${bonsaiSeleccionado._id}`, updatedProducto);
          ...
          <tbody>
            {catalogos.map((elemento) => (
              <tr>
                <td>{elemento._id}</td>
                <td>{elemento.tipo}</td>
                <td><img className="imagenArbol" src={`http://34.204.6.112:5000/${elemento.image}`} alt="arbolimagen"></img></td>
